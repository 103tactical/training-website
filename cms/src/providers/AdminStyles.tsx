  ...
  return (
    <>
      <style>{`
        ...
        /* ── Nav section ordering ─────────────────────────────────────────────
           The nav__wrap is a flex column. Assign explicit order values so sections
           always appear in the right sequence regardless of config array position.
           IDs are generated by Payload as: nav-group-${label} (spaces kept). */
        [id="nav-group-Course Management"] { order: 1; }
        [id="nav-group-Accounting & Reports"] { order: 2; }
        [id="nav-group-Pages"] { order: 3; }
        [id="nav-group-Data"] { order: 4; }
        .nav__controls { order: 999; }
